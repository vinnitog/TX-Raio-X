...
- O webhook de teste do Mercado Pago valida a assinatura HMAC antes de qualquer efeito, consulta o pagamento pela API e confere ambiente, vendedor, ordem, valor e moeda. Uma RPC transacional restrita ao `service_role` atualiza pagamento e ordem e concede ou reverte o pacote no ledger com idempotência; o frontend ainda não consulta esse saldo nem consome créditos server-side.
