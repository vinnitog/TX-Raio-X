...
- A Edge Function `checkout` foi publicada no ambiente de testes com autenticação manual; CORS, rejeição de sessão ausente/inválida e checkout autenticado real com crédito de 10 análises foram validados pelo site.
...
- O Railway está preparado como nova hospedagem estática por Docker/Caddy, com allowlist idêntica ao artefato público, porta dinâmica, `/health`, headers de segurança, cache compatível com o service worker e logs minimizados. GitHub Pages permanece durante a transição; secrets Stripe e `service_role` continuam exclusivamente no Supabase.
- O domínio Railway `tx-raio-x-production.up.railway.app` está registrado na allowlist versionada de redirects do Supabase Auth.
- O quality gate versionado executa unitários, jornadas Playwright e pgTAP em Postgres efêmero em pushes de `develop` e PRs para `main`. A `main` exige PR, check `quality-gate` estrito e conversas resolvidas, bloqueia force push/exclusão e inclui administradores; `develop` permanece sem proteção para o fluxo local.
